...
class MessageCordViewImpl(val parentView: PatcherView, val elem: Cord) extends CordViewImpl {
  val peer: dom.svg.Line = {
    ...
    line(cls := "pat-cord pat-message-cord").render
  }
...
class AudioCordViewImpl(val parentView: PatcherView, val elem: Cord) extends CordViewImpl {
  ...
  private val line1 = line(cls := "pat-cord pat-audio-cord1").render
  private val line2 = line(cls := "pat-cord pat-audio-cord2").render

  val peer: dom.svg.Element = g(cls := "pat-node", line1, line2).render
